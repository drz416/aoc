...
def main(rows: list[str]):
    ...
    # Map all distances
    distances = {}
    for id, ((box1, box2)) in enumerate(combinations(boxes, 2)):
        distance = ((box1[0]-box2[0])**2
            + (box1[1]-box2[1])**2
            + (box1[2]-box2[2])**2
        )**0.5
        distances[id] = (distance, box1, box2)

    # Isolate shortest connections
    if len(sys.argv) == 1:                  # ran on example input
        num_connections = 10
    else:                                   # ran on pizzle input
        num_connections = 1000
    shortest = sorted(tuple(distances.values()))[:num_connections]
    ...
    for _, box1, box2 in shortest:
        ...
        if (box1 in box_locations) and (box2 not in box_locations):
        ...
        if (box1 not in box_locations) and (box2 in box_locations):
        ...
        if (box1 in box_locations) and (box2 in box_locations):
    ...
    # Calc answer
    circuit_sizes: list[tuple[int]] = []
    for key, value in circuits.items():
        circuit_sizes.append((len(value), key))

    ans = 1
    for size, _ in sorted(circuit_sizes, reverse=True)[:3]:
        ans *= size

    # pprint(box_locations)
    # pprint(circuits)
    # pprint(sorted(circuit_sizes, reverse=True))
