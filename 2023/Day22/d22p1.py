...
1,1,8~1,1,9"""
    ...
    # Check disintegrations
    for brick in bricks.values():
        if "Ground" in brick.resting_on:
            continue
        if len(brick.resting_on) == 0:
            raise ValueError(f"Undefined rest position of {brick.name}: {brick.resting_on}")
        if len(brick.resting_on) == 1:
            bricks[brick.resting_on.copy().pop()].can_be_disintegrated = False
        else:
            for below_brick in brick.resting_on:
                bricks[below_brick].can_be_disintegrated = bricks[below_brick].can_be_disintegrated and True
...
class Brick():
    ...
    def fall(self, grid: dict, bricks: dict) -> None:
        ...
        return self.fall(grid, bricks)              # Re-run the fall until reaching ground on another brick
