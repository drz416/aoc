...
10456732"""
    ...
    for r, row in enumerate(base_grid):
        for c, element in enumerate(row):
            if element == 0:
                trails[(r+1, c+1)] = set()         #+1+1 are new positions after border is added
...
def search_trail(grid: list[list[int]], trails: dict[set], trailhead: tuple[int], curr_position: tuple[int]) -> None:
    if grid[curr_position[0]][curr_position[1]] == 9:
        trails[trailhead].add(curr_position) 
        return
